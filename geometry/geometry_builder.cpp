...
GeometryPath build_geometry_with_callback(
    ...
    // Initialize the running spline with the start position
    state.running_spline.add_waypoint(frames[0].position);
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        // Claim all crossover slots upfront (both entry and exit)
        for (SegmentId parent_id : seg.through) {
        ...
        if (seg.forms_loop) {
            ...
            if (precomp_it != precomputed_loops.end()) {
                ...
                // Fallback: entry passthroughs + connector to next
                for (SegmentId parent_id : seg.through) {
                    auto entry_it = claimed_entry_crossovers.find({parent_id, seg_id});
                    if (entry_it != claimed_entry_crossovers.end()) {
                ...
                add_waypoint_to_splines(state, segment_spline, next_pos,
                                        "loop_fallback_connector", on_curve_added);
            }
        } else {
            // Non-loop segment: entry passthroughs + waypoint to next position
            for (SegmentId parent_id : seg.through) {
                auto entry_it = claimed_entry_crossovers.find({parent_id, seg_id});
                if (entry_it != claimed_entry_crossovers.end()) {
                    build_parent_passthrough(state, segment_spline, entry_it->second, on_curve_added);
        ...
        log->debug("  segment {}: forms_loop={}, children={}, arc_length={:.3f}, has_parent={}",
    ...
    validate_geometry(result.segments_);
